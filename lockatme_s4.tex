...
aujourd'hui \em{Wayland} plutôt que \em{X}. Mais \em{X} détient toujours de loin la plus
grande part d'utilisation. De plus, les programmes codés pour \em{X} fonctionne
sur \em{Wayland} grâce au programme de compatibilité \em{XWayland}. Le contraire n'est pas
possible. 

C'est pourquoi nous avons décidé d'utiliser l'API de \em{X} pour notre projet. C'est par
...
\em{X} possède en réalité deux API. \em{Xlib} est très utilisée et plutôt
simple dans son design.
C'est elle que nous avons utilisés. L'API plus récente, \em{XCB}, présente un
...
On crée un objet \em{threading.Event} dans le programme principal. Tous les modules
